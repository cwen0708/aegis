...
if [ "$CURRENT_BRANCH" != "dev" ]; then
  echo "ERROR: 目前在 $CURRENT_BRANCH，切換到 dev"
  git checkout dev
fi
```
**必須在 dev 分支上開發。main 分支的改動會被 hot_update 的 `git reset --hard` 覆蓋，等於白做。**
...
8. **只在 dev 分支 commit** — main 分支的 commit 會被 hot_update 的 `git reset --hard` 覆蓋掉，等於白做
