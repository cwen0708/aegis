...
from app.core.idle_detector import is_system_idle, reset_idle_tracking, IDLE_CPU_THRESHOLD
...
def _reset_and_mock_pool():
    """重置 idle 追蹤並 mock ProcessPool"""
    reset_idle_tracking()
    with patch("app.core.session_pool.process_pool") as mock_pool:
...
class TestIsSystemIdle:
    ...
    @patch("app.core.idle_detector.get_system_metrics", return_value=_mock_metrics(cpu=79.9))
    def test_idle_at_cpu_boundary(self, mock_metrics, db_session):
        """CPU 剛好低於門檻 → 閒置"""
        assert is_system_idle(db_session) is True

    @patch("app.core.idle_detector.get_system_metrics", return_value=_mock_metrics(cpu=80.0))
