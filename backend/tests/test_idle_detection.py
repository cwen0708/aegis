...
class TestIsSystemIdle:
    ...
    @patch("app.core.cron_poller.get_system_metrics", return_value=_mock_metrics(cpu=10.0))
    ...
    @patch("app.core.cron_poller.get_system_metrics", return_value=_mock_metrics(cpu=90.0))
    def test_busy_when_cpu_high(self, mock_metrics, db_session):
        """CPU >= 80% → 非閒置"""
        assert is_system_idle(db_session) is False

    @patch("app.core.cron_poller.get_system_metrics", return_value=_mock_metrics(cpu=79.9))
    def test_idle_at_cpu_boundary(self, mock_metrics, db_session):
        """CPU 剛好低於門檻 → 閒置"""
        assert is_system_idle(db_session) is True

    @patch("app.core.cron_poller.get_system_metrics", return_value=_mock_metrics(cpu=80.0))
