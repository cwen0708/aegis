...
def post_task_hooks(idx, result, new_status, token_info, card_data, project_name,
    ...
    # 記錄 token 用量和成本估計
    _record_card_cost(idx, token_info, result.get("provider", ""))
...
def _record_card_cost(idx, token_info: dict, provider: str) -> None:
    """記錄卡片的 token 用量和成本估計"""
    if not token_info:
        return
    ...
    try:
        with Session(engine) as session:
            card_idx = session.get(CardIndex, idx.card_id)
            if card_idx:
                ...
                session.commit()
                logger.info(
                    f"[Cost] Card {idx.card_id}: "
                    f"input={input_tokens}, output={output_tokens}, "
                    f"cost=${cost:.4f} (cumulative: ${card_idx.estimated_cost_usd:.4f})"
                )
    except Exception as e:
        logger.warning(f"[Cost] Failed to record cost for card {idx.card_id}: {e}")
