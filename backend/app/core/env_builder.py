...
- .with_project_vars(project_id) → 注入專案環境變數（從 DB）
- .with_global_api_keys() → 注入全域 API key（從 SystemSetting）
...
class EnvironmentBuilder:
    ...
    def with_project_vars(self, project_id: Optional[int]) -> "EnvironmentBuilder":
        """注入專案環境變數（從 DB 讀取 ProjectEnvVar）"""
        if not project_id:
            return self
    ...
    def with_global_api_keys(self) -> "EnvironmentBuilder":
        """注入全域 API Key（從 SystemSetting 讀取）"""
        try:
